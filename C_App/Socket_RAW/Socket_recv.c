...
int main(int argc, char *argv[])
{
	...
	unsigned char *icmp = (unsigned char *)ip + sizeof(struct iphdr);
...
int i = 0;
    ...
    while (!done) {
		...
		if (ip->daddr != inet_addr("192.168.1.6")) continue;
		if ((++i % 2) != 0) continue;
printf("send\n");
		ip->saddr = inet_addr("192.168.1.6");
		ip->daddr = inet_addr("192.168.0.102");
		icmp[0] = 0;
		icmp[2] += 8;

		ip->check	= htons(checksum((unsigned short*)(ip), (sizeof(struct iphdr)/2)));

		struct sockaddr_in server;
		server.sin_family = AF_INET;
		server.sin_addr.s_addr = ip->daddr;
sleep(2);
		int send_len = sendto(sock_raw,ip,84,0,(const struct sockaddr*)&server,sizeof(server));
		if(send_len<0)
		{
